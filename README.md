...
## `resources-mcp-component` frontend migrated to ClojureScript (2026-09-07)

`appview/resources-mcp-component/svelte/` (SvelteKit) is gone. That component's
frontend is now ClojureScript — reagent + re-frame + `jp-go-dds`
(デジタル庁デザインシステム) — at
[`appview/resources-mcp-component/cljs/`](appview/resources-mcp-component/cljs).
...
`appview/resources-mcp-component/wrangler.jsonc`'s header comment and
...
Two fields in `cljs/src/resources/app.cljs`'s `default-db` were also
...
```bash
cd appview/resources-mcp-component/cljs
npm install
